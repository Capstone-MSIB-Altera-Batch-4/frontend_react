...
const Products = () => {
  ...
  const category = useSelector((state) => state.products.category.data);
  ...
  // const options = category.map(item => item.name);
  // options.unshift("All");
  ...
  useEffect(() => {
    if (category?.length > 0) {
      const categoryOptions = category.map(item => item.name);
